...
export function useNotes() {
	...
	const createNote = useCallback(async (params: NoteApiRequest, targetDate: Date): Promise<void> => {
		...
		try {
			...
			const errorMessage = err instanceof ApiResponseError ? err.message : "ノートの作成に失敗しました";
	...
	const updateNote = useCallback(async (noteId: string, params: NoteApiRequest, targetDate: Date): Promise<void> => {
		const { content, accessLevel, images, tagIds } = params;
		if (!content) return;
		...
		try {
			...
			const errorMessage = err instanceof ApiResponseError ? err.message : "ノートの編集に失敗しました";
			setError(errorMessage);
			throw err;
		} finally {
			setLoading(false);
	...
	const deleteNote = useCallback(async (noteId: string, targetDate: Date): Promise<void> => {
		setLoading(true);
		setError(null);
		...
		try {
			...
			const errorMessage = err instanceof ApiResponseError ? err.message : "ノートの削除に失敗しました";
			setError(errorMessage);
			throw err;
		} finally {
			setLoading(false);
